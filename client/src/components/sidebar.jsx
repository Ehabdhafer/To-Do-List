...
const Sidebar = () => {
    ...
    useEffect(() => {
      const fetchTaskCount = async () => {
        try {
          const response = await axios.get("http://localhost:8000/taskcount");
      ...
      const todayTaskCount = async () => {
        try {
          const response = await axios.get("http://localhost:8000/todaytaskcount");
          setTodayTaskCount(response.data);
        } catch (error) {
          console.error("Error fetching task count:", error);
        }
      };
    ...
    return (
      ...
      <button
                ...
                onClick={handleMenu}
            >
                <span className="sr-only">{menuopen ? 'Close sidebar' : 'Open sidebar'}</span>
                <svg
                    className="w-6 h-6"
                    aria-hidden="true"
                    fill="currentColor"
            ...
            <aside
                id="logo-sidebar"
                className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${menuopen ? 'translate-x-0' : '-translate-x-full'} xl:translate-x-0`}
                aria-label="Sidebar"
            >
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
                    <button
                        type="button"
                        className="absolute top-0 right-0 mt-4 mr-4 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-300"
                    ...
                    className="flex items-center ps-2.5 mb-5">
                        <img
                            ...
                            className="h-10 w-14 me-3 sm:h-12"
                            alt="To Do list Logo"
                        />
            ...
            className={`${!user && "hidden"} block`}>
              ...
              >
                ...
                >
                  ...
                  <g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12ZM12 18C11.4477 18 11 17.5523 11 17V13H7C6.44772 13 6 12.5523 6 12C6 11.4477 6.44772 11 7 11H11V7C11 6.44772 11.4477 6 12 6C12.5523 6 13 6.44772 13 7V11H17C17.5523 11 18 11.4477 18 12C18 12.5523 17.5523 13 17 13H13V17C13 17.5523 12.5523 18 12 18Z" fill="#ff0000"></path> </g></svg>
            ...
            className={`${!user && "hidden"} block`}>
              ...
              >
                ...
                <span className="flex-1 ms-3 whitespace-nowrap">Due Today</span>
                <span className="inline-flex items-center justify-center w-3 h-3 p-3 ms-3 text-sm font-medium text-blue-800 bg-blue-100 rounded-full dark:bg-blue-900 dark:text-blue-300">
                  {todayTaskCount}
                </span>
              </button>
